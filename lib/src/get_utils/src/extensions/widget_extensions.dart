...
extension WidgetExtensions on Widget {
  ...
  Widget paddingOnly({
    ...
    assert(left >= 0 && top >= 0 && right >= 0 && bottom >= 0, 'Padding must be non-negative');
  ...
  Widget marginOnly({
    ...
    assert(left >= 0 && top >= 0 && right >= 0 && bottom >= 0, 'Margin must be non-negative');
    return Container(margin: EdgeInsets.only(top: top, left: left, right: right, bottom: bottom), child: this);
  }
  ...
  Widget backgroundColor(Color color, {double opacity = 1.0}) {
    ...
    return Opacity(opacity: opacity, child: Container(color: color, child: this));
  }
...
extension WidgetResponsiveX on Widget {
  ...
  Widget centered({bool horizontal = true, bool vertical = true}) => Center(widthFactor: horizontal ? null : 1.0, heightFactor: vertical ? null : 1.0, child: this);
}
