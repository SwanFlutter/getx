...
// Controller with RxList
class UserController extends GetxController {
  // Basic RxList for users
  final users = <User>[].obs;
  
  // RxList for filtered users
  final filteredUsers = <User>[].obs;
  
  // RxInt for age filter
  final ageFilter = 0.obs;
  ...
  void _applyFilter() {
    if (ageFilter.value == 0) {
      ...
      filteredUsers.assignAll(
        users.where((user) => user.age >= ageFilter.value)
      );
    }
  }
  
  // Sort users by age
  void sortByAge({bool descending = false}) {
    users.sort((a, b) => 
      descending ? b.age.compareTo(a.age) : a.age.compareTo(b.age)
    );
  }
...
// Example Widget using GetX and RxList
      ...
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton(
            itemBuilder: (context) => [
              PopupMenuItem(
                child: Text('Sort by Age (Asc)'),
                onTap: () => controller.sortByAge(),
              ),
              PopupMenuItem(
                child: Text('Sort by Age (Desc)'),
                onTap: () => controller.sortByAge(descending: true),
              ),
              PopupMenuItem(
              ...
              PopupMenuItem(
                ...
                onTap: () => controller.sortByName(descending: true),
              ),
            ],
          ),
        ],
      ...
      body: Column(
        children: [
          ...
          Padding(
            ...
            child: Obx(() => Column(
              children: [
                Text('Age Filter: ${controller.ageFilter.value}'),
                Slider(
                  value: controller.ageFilter.value.toDouble(),
                  min: 0,
                  max: 100,
                  divisions: 100,
                  onChanged: (value) => 
                    controller.updateAgeFilter(value.toInt()),
                ),
              ],
            )),
          ),
...
// Example 2: Shopping Cart Controller
class Product {
  final String name;
  final double price;
...
// Example 3: Todo List with Categories
class Todo {
  final String title;
  final String category;
  final bool isDone;
  
  Todo(this.title, this.category, {this.isDone = false});
  
  Todo copyWith({String? title, String? category, bool? isDone}) {
    return Todo(
      title ?? this.title,
...
class TodoController extends GetxController {
  ...
  final categories = <String>{'Work', 'Personal', 'Shopping'}.obs;
  
  // Filtered todos by category
  ...
  Map<String, int> get statistics {
    return {
      'total': todos.length,
      'completed': todos.where((todo) => todo.isDone).length,
      'pending': todos.where((todo) => !todo.isDone).length,
    };
  }
}
